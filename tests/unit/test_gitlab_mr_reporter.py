"""Unit tests for reporters/gitlab_mr.py."""
...
def _make_result(
    ...
    *,
    ...
    return ReviewResult(
        ...
        target=ReviewTarget(type="pr", pr_url="https://gitlab.com/g/p/-/merge_requests/42"),
    )
...
class TestGitLabMRReporterEnabled:
    def test_is_enabled_returns_true(self) -> None:
        scm = MagicMock(spec=GitLabSCM)
        reporter = GitLabMRReporter(scm=scm, mr_identifier="g/p!42")
        assert reporter.is_enabled() is True


class TestGitLabMRReporterReport:
    @pytest.mark.asyncio
    async def test_no_findings_posts_only_summary(self) -> None:
        scm = AsyncMock(spec=GitLabSCM)
        reporter = GitLabMRReporter(scm=scm, mr_identifier="g/p!42")
        result = _make_result()
    ...
    async def test_inline_comment_failure_does_not_stop_report(self) -> None:
        ...
        # Summary should still be posted despite inline failure
        scm.post_review_summary.assert_called_once()
