"""Unit tests for reporters/bitbucket_pr.py."""
...
class TestBitbucketPRReporterEnabled:
    def test_is_enabled_returns_true(self) -> None:
        scm = MagicMock(spec=BitbucketSCM)
        reporter = BitbucketPRReporter(scm=scm, pr_identifier="workspace/repo#42")
        assert reporter.is_enabled() is True


class TestBitbucketPRReporterReport:
    @pytest.mark.asyncio
    async def test_no_findings_posts_only_summary(self) -> None:
        scm = AsyncMock(spec=BitbucketSCM)
        reporter = BitbucketPRReporter(scm=scm, pr_identifier="workspace/repo#42")
        result = _make_result()
    ...
    @pytest.mark.asyncio
    async def test_high_findings_request_changes(self) -> None:
        scm = AsyncMock(spec=BitbucketSCM)
        reporter = BitbucketPRReporter(scm=scm, pr_identifier="workspace/repo#42")
        finding = _make_finding(severity=Severity.HIGH, title="High severity bug")
        result = _make_result(findings=(finding,))
