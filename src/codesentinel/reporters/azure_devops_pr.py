"""Azure DevOps PR reporter — posts findings as PR comment threads."""
...
class AzureDevOpsPRReporter(Reporter):
    """Post review findings as Azure DevOps PR comment threads.

    Each finding becomes an inline comment thread on the affected file/line.
    A summary thread aggregates all findings at the PR level.
    """
    ...
    async def report(self, result: ReviewResult) -> None:
        """Post all findings as inline threads plus a summary thread."""
    ...
    async def _post_inline_comment(self, finding: Finding) -> None:
        """Post a single finding as an inline PR comment thread."""
    ...
    async def _post_summary(self, result: ReviewResult) -> None:
        """Post a summary thread on the PR with all findings aggregated."""
